...
API Key 保存在 Windows Credential Manager；Endpoint、模型名、提供方、场合、语气和快捷键保存在 `%LocalAppData%\KEYI\settings.json`。旧版设置会在首次启动时按兼容规则导入；已有 KEYI 设置始终优先。旧 Credential Manager 目标名不会自动迁移，需重新输入一次 API Key。
